...
namespace CLFramework
{
    public class PacketWriter
    {
        ...
        {
            //Console.WriteLine($"DataValue = {data} -> DatTypeWORD = {data.GetType().ToString()}");

            if (Convert.ToInt32(data) < 0)
            {
                bw.Write(((IConvertible)data).ToInt16(null));
            }
            else
            {
                bw.Write(((IConvertible)data).ToUInt16(null));
            }
        }
        public void DWord(object data)
        {
            //data.GetType().ToString() == "System.Int32" && Convert.ToInt32(data);
            //Console.WriteLine($"DataValue = {data} -> DatType_DWord = {data.GetType().ToString()}");
            if (Convert.ToInt32(data) < 0)
            {
