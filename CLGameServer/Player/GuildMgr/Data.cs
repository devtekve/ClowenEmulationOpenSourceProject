...
namespace CLGameServer
{
    public partial class PlayerMgr
    {
        ...
        public void _UpdateGuildLocation(byte xsec, byte ysec)
        {
            //If character is in a guild
            if (Character.Network.Guild.Guildid != 0)
            {
                if (xsec != Character.Position.xSec || ysec != Character.Position.ySec)
                {
                    //Repeat for each client in the guild
                    foreach (SRClient memberclient in Character.Network.Guild.MembersClient)
                    {
