...
namespace CLGameServer.Client
{
    public partial class Packet
    {
        public static byte[] SendGuildStart()
        {
            ...
            Writer.Create(OperationCode.SERVER_GUILD_INFO_START);
        ...
        {
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_ICON_SEND);
            Writer.Byte(type);
            Writer.Text(icon);
            return Writer.GetBytes();
        }
        public static byte[] PromoteOrDisband(byte information)
        {
            ...
            switch (information)
            {
                case 1:
                    //Disband message
                    Writer.Create(OperationCode.SERVER_GUILD_PROMOTE_MSG);
                    Writer.Byte(1);
                    break;
                case 2:
                    //Promote upgrade message
                    Writer.Create(OperationCode.SERVER_GUILD_DISBAND_MSG);
                    Writer.Byte(1);
                    break;
            }
            return Writer.GetBytes();
        ...
        public static byte[] SendGuildEnd()
        {
            ...
            /////////////////////////////////////////////////////////////////////////
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_INFO_END);
            return Writer.GetBytes();
        }
        ...
        {
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_WAR_MSG);
            switch (type)
            {
                case 2:
                    Writer.Byte(type);
        ...
        public static byte[] GuildStorageStart(byte type)
        {
            ...
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_STORAGE);
            switch (type)
            {
                case 1:
                    //Success opening
        ...
        {
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_STORAGE4);
            Writer.Byte(3);
            Writer.Byte(4);
            return Writer.GetBytes();
        }
        ...
        {
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_STORAGE_END);
            return Writer.GetBytes();
        }
        public static byte[] GuildStorageClose()
        {
            PacketWriter Writer = new PacketWriter();
            Writer.Create(OperationCode.SERVER_GUILD_STORAGE_CLOSE);
            Writer.Byte(1);
            return Writer.GetBytes();
        }
