...
namespace CLGameServer
{
    public partial class PlayerMgr
    {
        public void SetWaitTime()
        {
            ...
            DB.query("UPDATE character SET GuildJoining='1',GuildTime='" + DateTime.Now.AddDays(3) + "' WHERE name='" + Character.Information.Name + "'");
            Character.Information.GuildPenalty = true;
        }
        ...
        public void GuildDisband()
        {
            try
            {
                foreach (int member in Character.Network.Guild.Members)
                {
                    if (member != 0)
                    {
                        PlayerMgr guildplayer = Helpers.GetInformation.GetPlayerMainid(member);
                        if (guildplayer != null)
                        {
