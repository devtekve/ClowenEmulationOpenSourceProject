    ...
    public partial class PlayerMgr
    {
        ...
        void Player_MoveItemToStorage(byte f_slot, byte t_slot, int o_id)
        {
            ...
            try
            {
                ...
                if (!Character.Network.Guild.UsingStorage)
                {
                    ...
                    int storageprice = ObjData.Manager.ItemBase[item.ID].Storage_price;
                    ...
                    int ownerid = Convert.ToInt32(DB.GetData("SELECT * FROM char_items WHERE id='" + item.dbID + "'", "owner"));
                    ...
                    if (ownerid == Character.Information.CharacterID)
                    {
                        ...
                        if (item.Amount <= ObjData.Manager.ItemBase[item.ID].Max_Stack)
                        {
                            ...
                            if (Character.Information.Gold >= storageprice)
                            {
                                ...
                                client.Send(Packet.UpdateGold(Character.Information.Gold));
                                //Update DB database information
                                DB.query("UPDATE char_items SET itemnumber='item" + t_slot + "',slot='" + t_slot + "',storagetype='1' WHERE storageacc='" + Player.ID + "' AND id='" + item.dbID + "'");
                    ...
                    if (ownerid == Character.Information.CharacterID)
                    {
                        ...
                        if (item.Amount <= ObjData.Manager.ItemBase[item.ID].Max_Stack)
                        {
                            ...
                            if (Character.Information.Gold >= storageprice)
                            {
                                ...
                                client.Send(Packet.UpdateGold(Character.Information.Gold));
                                //Update DB database information
                                DB.query("UPDATE char_items SET itemnumber='item" + t_slot + "',slot='" + t_slot + "',storagetype='3' ,guild_storage_id='" + Character.Network.Guild.Guildid + "' WHERE id='" + item.dbID + "'");
                                //Send visual move packet to player
            ...
            try
            {
                ...
                if (!Character.State.Die || !Character.Stall.Stallactive || !Character.Alchemy.working || !Character.Position.Walking)
                {
                    ...
                    if (!Character.Network.Guild.UsingStorage)
                    {
                        ...
                        ObjData.slotItem item = GetItem((uint)Player.ID, f_slot, 1);
                        //Cannot drag and drop onto equip slots.
                        if (t_slot < 13) return;
                        //Update database information
                        DB.query("UPDATE char_items SET itemnumber='item" + t_slot + "',slot='" + t_slot + "',storagetype='0', owner=" + Character.Information.CharacterID + " WHERE storageacc='" + Player.ID + "' AND id='" + item.dbID + "'");
                        //Send visual packet
                    ...
                    else
                    {
                        ...
                        byte freeslot = GetFreeSlot();
                        //Get item information the one that user selected to move.
                        ObjData.slotItem item = GetItem((uint)Player.ID, f_slot, 3);
                        //Cannot drag and drop onto equip slots.
                        if (t_slot < 13) return;
                        //Update database information
                        DB.query("UPDATE char_items SET itemnumber='item" + t_slot + "',slot='" + t_slot + "',storagetype='0', owner=" + Character.Information.CharacterID + " WHERE guild_storage_id='" + Character.Network.Guild.Guildid + "' AND id='" + item.dbID + "'");
                        //Send visual packet
        ...
        void ItemMoveInStorage(byte fromSlot, byte toSlot, short quantity)
        {
            ...
            try
            {
                ...
                ObjData.slotItem fromItem = GetItem((uint)Character.Information.CharacterID, fromSlot, 1);
                //Check item where we are moving to
                ObjData.slotItem toItem = GetItem((uint)Character.Information.CharacterID, toSlot, 1);
                ...
                if (!Character.Network.Guild.UsingStorage)
                {
                    if (toItem.ID != 0)
                    {
                        ...
                        DB.query("UPDATE char_items SET itemnumber='item" + toSlot + "',slot='" + toSlot + "' WHERE itemnumber='" + "item" + fromSlot + "' AND owner='" + Player.ID + "' AND itemid='" + fromItem.ID + "' AND id='" + fromItem.dbID + "' AND storagetype='" + storagetype + "'");
                        //To item database update
                        DB.query("UPDATE char_items SET itemnumber='item" + fromSlot + "',slot='" + fromSlot + "' WHERE itemnumber='" + "item" + toSlot + "' AND owner='" + Player.ID + "' AND itemid='" + toItem.ID + "' AND id='" + toItem.dbID + "' AND storagetype='" + storagetype + "'");
                    }
                    else
                    {
                        client.Send(Packet.MoveItem(0, fromSlot, toSlot, quantity, 0, "MOVE_INSIDE_STORAGE"));
                        DB.query("UPDATE char_items SET itemnumber='item" + toSlot + "',slot='" + toSlot + "' WHERE itemnumber='" + "item" + fromSlot + "' AND owner='" + Player.ID + "' AND itemid='" + fromItem.ID + "' AND storagetype='"+ storagetype +"'");
                    }
                }
                //Guild storage
                else
                {
                    ...
                    if (toItem.ID != 0)
                    {
                        ...
                        //First we update database with the 2 items (From item).
                        DB.query("UPDATE char_items SET itemnumber='item" + toSlot + "',slot='" + toSlot + "' WHERE itemnumber='" + "item" + fromSlot + "' AND guild_storage_id='" + Character.Network.Guild.Guildid + "' AND itemid='" + fromItem.ID + "' AND id='" + fromItem.dbID + "' AND storagetype='" + storagetype + "'");
                        //To item database update
                        DB.query("UPDATE char_items SET itemnumber='item" + fromSlot + "',slot='" + fromSlot + "' WHERE itemnumber='" + "item" + toSlot + "' AND guild_storage_id='" + Character.Network.Guild.Guildid + "' AND itemid='" + toItem.ID + "' AND id='" + toItem.dbID + "' AND storagetype='" + storagetype + "'");
                        
                    }
                    else
                    {
                        client.Send(Packet.MoveItem(0, fromSlot, toSlot, quantity, 0, "MOVE_INSIDE_GUILD_STORAGE"));
                        DB.query("UPDATE char_items SET itemnumber='item" + toSlot + "',slot='" + toSlot + "' WHERE itemnumber='" + "item" + fromSlot + "' AND guild_storage_id='" + Character.Network.Guild.Guildid + "' AND itemid='" + fromItem.ID + "' AND storagetype='"+ storagetype +"'");
                    }
                }
            }
            catch (Exception ex)
        ...
        void ItemMoveExchangeToInv(byte f_slot)
        {
            ...
            ObjData.slotItem wannadeleteitem = Character.Network.Exchange.ItemList[f_slot];
